...
class PredictThread(QThread):
    def __init__(self,day,model,state,mainpage):
        super().__init__()
        self.day=day
        if(model=="线性回归模型"):
            self.model=myLinearRegressionModel("cases")
            self.modelpath="./model/lr_model_"+states[state]
        elif(model=="随机森林回归模型"):
            self.model=myRandomForestModel("cases")
            self.modelpath="./model/rf_model_"+states[state]
        elif(model=="梯度上升决策树回归模型"):
            self.model=myGBTRegressionModel("cases")
            self.modelpath="./model/gbt_model_"+states[state]
        self.state=state
        self.mainpage=mainpage
    def run(self):
        data=None
        if(self.state=="美国全境"):
            data=self.mainpage.spark.sql("select date,sum(cases) cases from table group by date order by date asc")
        else:
            data=self.mainpage.spark.sql("select date,sum(cases) cases from table where state={zhou} group by date order by date asc",zhou=states[self.state])
        data = self.model.load_and_prepare_data(data)
        if os.path.exists(self.modelpath):
            self.model.load_model(self.modelpath)
            predict=self.model.make_predictions(self.day)
            self.model.plot_predictions(data,predict)
        else:
            self.model.train_model(data)
            predeict=self.model.make_predictions(self.day)
            self.model.plot_predictions(data,predeict)
            self.model.save_model(self.modelpath)
        self.model.stop_spark()
...
class MyMainWindows(QMainWindow, Ui_Form):
    ...
    def predict(self):
        ...
        self.thread=PredictThread(day,model,state,self)
        self.thread.start()
